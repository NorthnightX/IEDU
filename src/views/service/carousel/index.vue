<template>
  <div class="app-container">
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          icon="Plus"
          @click="handleAdd"
          v-hasPermi="['service:carousel:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['service:carousel:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['service:carousel:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          icon="Download"
          @click="handleExport"
          v-hasPermi="['service:carousel:export']"
        >导出</el-button>
      </el-col>
    ...
    <el-table v-loading="loading" :data="carouselList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="图片链接" align="center" prop="eduLink">
        <template #default="scope">
          <image-preview :src="scope.row.eduLink"></image-preview>
        </template>
      </el-table-column>
      ...
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
        <template #default="scope">
          <el-button link type="primary" icon="Edit" @click="handleUpdate(scope.row)" v-hasPermi="['service:carousel:edit']">修改</el-button>
          <el-button link type="primary" icon="Delete" @click="handleDelete(scope.row)" v-hasPermi="['service:carousel:remove']">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
...
import { listCarousel, getCarousel, delCarousel, addCarousel, updateCarousel } from "@/api/service/carousel";
...
function handleUpdate(row) {
  ...
  getCarousel(_eduId).then(response => {
...
function handleDelete(row) {
  ...
  proxy.$modal.confirm('是否确认删除轮播图管理编号为"' + _eduIds + '"的数据项？').then(function() {
    return delCarousel(_eduIds);
  }).then(() => {
    getList();
...
function handleExport() {
  proxy.download('service/carousel/export', {
    ...queryParams.value
